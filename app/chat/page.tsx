              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    <span className="font-medium text-gray-100">{chat.character.name}</span>
          ...
          {showCharacterSelection && (
            <div className="absolute inset-0 bg-black/95 flex flex-col items-center justify-start z-50 p-6 pt-16 overflow-y-auto">
              ...
              <div className="w-full max-w-6xl flex flex-col gap-10">
                {letters.length === 0 && <p className="text-gray-500 text-center">No characters found.</p>}
                {letters.map((letter) => (
                  <div key={letter}>
                    <h3 className="text-2xl font-semibold mb-4 text-gray-300 tracking-wide">{letter}</h3>
                    <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      {groupedCharacters[letter].map((char) => (
                        ...
                        >
                          ...
                          />
                          <span className="text-lg font-medium text-gray-200">{char.name}</span>
                        </button>
                      ))}
                    </div>
                  </div>
